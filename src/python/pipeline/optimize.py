...
def get_regime_at_date(regimes: pd.DataFrame, date: pd.Timestamp) -> dict[str, float | int]:
    """Return latest regime information available at or before a date.

    Parameters
    ----------
    regimes : pd.DataFrame
        Regimes DataFrame containing at least ``date`` and ``state``.
        Probability columns (``p_state_*``) are optional.
    date : pd.Timestamp
        Rebalance date for lookup.

    Returns
    -------
    dict[str, float | int]
        Dict containing ``date``, ``state`` and optional state probabilities.

    Raises
    ------
    ValueError
        If no valid row is available at or before the requested date.
    """
...
def regime_policy_from_state(state: int, cfg: OptimizeConfig) -> dict[str, float]:
    """Map a regime state to a deterministic risk policy.

    The mapping currently scales gross risk and concentration limits:
    state ``0`` is neutral, ``1`` is defensive, others are most defensive.
    ...
    -------
    dict[str, float]
        Policy with ``risk_scale``, ``max_weight_scale`` and ``base_max_weight``.
    """
    if state is None:
...
def apply_regime_policy(
    ...
    """Apply regime policy scaling and concentration cap to weights.
    ...
    weights : np.ndarray
        Input weight vector.
    policy : dict[str, float]
        Policy generated by :func:`regime_policy_from_state`.
    allow_cash : bool
        If False, renormalize adjusted weights to sum to 1.

    Returns
    -------
    np.ndarray
        Adjusted weights after risk scaling and max-weight clipping.
    """
...
def get_mc_risk_at_date(
    ...
    """Return latest MC summary at or before a date for one horizon.

    Parameters
    ----------
    mc : pd.DataFrame
        Monte Carlo summary dataset with ``date``, ``horizon``, ``var``, ``cvar``, ``q95``.
    date : pd.Timestamp
        Rebalance date for lookup.
    horizon : int
        Simulation horizon to filter.

    Returns
    -------
    dict[str, float]
        Dict with ``date``, ``state``, ``var``, ``cvar`` and ``q95``.

    Raises
    ------
    ValueError
        If no valid row is available for the requested horizon/date.
    """
...
def apply_mc_overlay(
    ...
    """Scale portfolio exposure when MC tail risk breaches configured limits.
    ...
    weights : np.ndarray
        Input weight vector.
    mc_summary : dict[str, float]
        Monte Carlo risk metrics (expects ``var`` and ``cvar``).
    risk_limits : dict[str, float]
        Absolute risk limits, keys ``var`` and ``cvar``.
    allow_cash : bool
        If False, renormalize adjusted weights to sum to 1.
